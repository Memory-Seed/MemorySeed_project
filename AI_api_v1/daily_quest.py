...
class DailyQuestResult:
    ...
    def summary(self) -> str:
        ...
        for q in self.quests:
            ...
            lines.append(
                f"  [{q.difficulty.upper()} / {q.coin_reward}코인]{tag} {q.title}{target}\n"
                f"    -> {q.description}"
            )
        if self.hidden_quests:
            ...
            for q in self.hidden_quests:
                target = f" (목표: {int(q.target_value)}{q.target_unit})" if q.target_value and q.target_unit else ""
                lines.append(
                    f"  [HIDDEN / {q.coin_reward}코인] {q.title}{target}\n"
                    f"    -> {q.description}"
                )
        return "\n".join(lines)
...
class DailyQuestGenerator:
    ...
    def generate(
        ...
        today_cal     = self.pp._calendar(target)
        today_weather = today_summary.get("weather")
        tomorrow_cal  = self.pp._calendar(target + timedelta(days=1))
